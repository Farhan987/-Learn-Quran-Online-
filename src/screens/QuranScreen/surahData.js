...
class SurahData extends Component {
  componentDidMount() {
    const StudentID = this.props.navigation.getParam(
      "Student",
      "student-default"
    );
    ...
    if (this.state.IsMaked === true) {
      this.props.markAyah(
        ...
        new MyIssueMistake(
          ...
          this.props.studentIDforMistakMarking
        ),
        this
      );
    } else {
  ...
  setCount = () => {
    if (this.state.counterPage >= 604) {
      Alert.alert("Page not exist");
    } else {
      this.scrollToIndex1(this.state.counterPage);
    }
    if (this.state.counterPage <= 1) {
      Alert.alert("Page not exist");
    }
  };
  render() {
      ...
      <Container style={styles.container}>
        ...
        <FlatList
          ...
          renderItem={({ item, index }) => (
            <QuranPage
              srceenNavigation={this.props.BackToPrevious}
        ...
        <View style={{ height: 50, flexDirection: "row" }}>
          ...
          >
            ...
            >
              <Input
                defaultValue={JSON.stringify(this.state.counterPage)}
                keyboardType="numeric"
                placeholder="P.No"
                value={JSON.stringify(this.state.counterPage)}
                style={{ color: LIGHT_BROWN }}
                onChangeText={text => this.setState({ counterPage: text })}
              >
...
const mapStateToProps = state => ({
  ...
  studentIDforMistakMarking: state.TeacherReducer.studentID
});
