...
export const showQuarnPageWise = startPageNo => dispatch => {
  axios
    .get(
      "https://search-quran.herokuapp.com/quran/search/page/" + startPageNo + ""
    )
    .then(res => {
...
export const setPageMistake = (pageIdex, mistakes) => dispatch => {
  dispatch({
    type: "LOAD_MISTAKES_",
    payload: { index: pageIdex, mmistakes: mistakes }
  });
};
  ...
  axios
    ...
    .then(res => {
      const state = {
        type: SET_QURAN_PAGE,
        payload: {
          pages: new QuranPages()
            .setAyats(pageNo, res.data.ayahs)
            .setLoaded(true),
          mistakes: mistakes
        }
      };
      dispatch(state);
      // console.log(`downloded... page: ${pageNo} `);
      // context.setState({ progress: context.state.progress + 1 }, () => {
      //   if (context.state.progress === 604) {
      //     context.moveToStudent();
      //   }
      // });
    })
    .catch(err => {
