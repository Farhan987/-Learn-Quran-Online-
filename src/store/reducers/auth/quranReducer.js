...
function AppReducer(mState = { ...state }, action) {
  switch (action.type) {
    ...
    case "LOADMYSTATE":
      ...
      console.log(mState.completeQuran);
      mState.totalPages = mState.completeQuran.length;
      console.log(":OADMYSTATE =========");
      console.log(mState);
...
const saveState = state => {
  AsyncStorage.setItem("quranReducer0101", JSON.stringify(state));
};

const loadState = () => {
  AsyncStorage.getItem("quranReducer0101")
    .then(state => {
      AppReducer(JSON.parse(state), { type: "LOADMYSTATE" });
    })
    .catch(err => {
      console.log(err);
    });
